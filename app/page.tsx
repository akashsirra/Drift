...
export default function Home(){
 ...
 const streamLinks=(items:Stream[],title:string)=> <div className="streams">{items.map((s,i)=>s.url?<a key={i} href={"/watch?url="+encodeURIComponent(s.url)+"&title="+encodeURIComponent(title)} className="stream">{s.name||s.title||"Play"} ▶</a>:s.externalUrl?<a key={i} href={s.externalUrl} target="_blank" rel="noreferrer" className="stream">{s.name||s.title||"Open external"} ↗</a>:<span key={i} className="stream">{s.name||s.title||s.infoHash||"Unsupported stream transport"}</span>)}</div>;
 return <main>
