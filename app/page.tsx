...
function AddonSection({addon,query,onOpen}:{addon:Addon;query:string;onOpen:(a:Addon,m:Meta)=>void}){
 const [items,setItems]=useState<Meta[]>([]),[typeFilter,setTypeFilter]=useState<"movie"|"series">("movie");
 useEffect(()=>{let dead=false;setItems([]);const load=async()=>{for(const c of (addon.catalogs||[]).filter(c=>c.type===typeFilter)){if(query&&!c.extra?.some(e=>e?.name==="search"))continue;const extra=query?"&search="+encodeURIComponent(query):"";const r=await fetch("/api/addon/resource?addon="+encodeURIComponent(addon.url)+"&resource=catalog&type="+c.type+"&id="+encodeURIComponent(c.id)+extra);if(r.ok){const j=await r.json();if(!dead)setItems(x=>[...x,...(j.metas||[]).slice(0,20)])}}};load();return()=>{dead=true}},[addon.url,query,typeFilter]);
