...
function AddonSection({addon,query,onOpen}:{addon:Addon;query:string;onOpen:(a:Addon,m:Meta)=>void}){
 const [items,setItems]=useState<Meta[]>([]),[homeItems,setHomeItems]=useState<Meta[]>([]),[typeFilter,setTypeFilter]=useState<"movie"|"series">("movie"),[loading,setLoading]=useState(false),[catalogError,setCatalogError]=useState("");
 useEffect(()=>{let dead=false;setCatalogError("");const q=query.trim();if(!q&&homeItems.length){setItems(homeItems);setLoading(false);return()=>{dead=true}}setItems([]);setLoading(true);const load=async()=>{try{const catalogs=(addon.catalogs||[]).filter(c=>c.type===typeFilter&&(!q||c.extra?.some(e=>e?.name==="search")));const results=await Promise.all(catalogs.map(async c=>{const extra=q?"&search="+encodeURIComponent(q):"";try{const r=await fetch("/api/addon/resource?addon="+encodeURIComponent(addon.url)+"&resource=catalog&type="+c.type+"&id="+encodeURIComponent(c.id)+extra+"&_drift_catalog="+Date.now(),{cache:"no-store"});if(!r.ok)return[];const j=await r.json();return Array.isArray(j.metas)?j.metas:[]}catch{return[]}}));if(!dead){const seen=new Set<string>();const merged=results.flat().filter((m:Meta)=>m?.id&&!seen.has(m.id)&&seen.add(m.id)).slice(0,24);setItems(merged);if(!q)setHomeItems(merged)}}catch(e){if(!dead)setCatalogError(e instanceof Error?e.message:"Catalog failed")}finally{if(!dead)setLoading(false)}};load();return()=>{dead=true}},[addon.url,query,typeFilter,homeItems.length]);
