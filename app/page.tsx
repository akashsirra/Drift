...
export default function Home(){
 const [addons,setAddons]=useState<Addon[]>([]),[url,setUrl]=useState(""),[loading,setLoading]=useState(false),[error,setError]=useState(""),[notice,setNotice]=useState(""),[selected,setSelected]=useState<Meta|null>(null),[streams,setStreams]=useState<Stream[]>([]),[selectedSeason,setSelectedSeason]=useState(1),[query,setQuery]=useState(""),[testId,setTestId]=useState(""),[testType,setTestType]=useState<"movie"|"series">("movie"),[testing,setTesting]=useState(false),[resolving,setResolving]=useState(false);
 ...
 async function openMeta(a:Addon,m:Meta){
  setSelected(m);setSelectedSeason(1);setStreams([]);setError("");setNotice("Resolving streams…");setResolving(true);
  try{
 ...
 return <main>
  <header><a href="/" className="brand">DRIFT</a><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search your addons..."/><a className="nav" href="/library">Library</a><a className="nav" href="/downloads">Downloads</a><a className="nav" href="/addons">Addons</a></header>
  ...
  {selected&&<div className="modal"><div className="modalCard"><button className="close" onClick={()=>setSelected(null)}>×</button><div className="detail">{selected.poster&&<img src={selected.poster}/>}<div><span>{selected.type}</span><h2>{selected.name}</h2><p>{selected.description}</p><p>{selected.releaseInfo}</p><div className="actions"><button onClick={library}>＋ Library</button></div>{selected.type==="series"&&selected.videos?.length?<EpisodePicker meta={selected} season={selectedSeason} setSeason={setSelectedSeason} onEpisode={openEpisode}/>:null}<div className="streamStatus">{resolving?"⏳ Resolving streams…":notice||"No stream information yet."}</div>{error&&<div className="error">{error}</div>}{!resolving&&!streams.length&&<button className="resolveAgain" onClick={()=>openMeta(addons.find(x=>x.url===selected.id)?.url?addons.find(x=>x.url===selected.id)!:addons.find(x=>Array.isArray(x.resources)&&x.resources.some((r:any)=>r==="meta"))||addons[0],selected)} disabled={!addons.length}>↻ Resolve streams again</button>}{streamLinks(streams,selected.name,selected)}</div></div></div></div>}
 </main>
}
...
function AddonSection({addon,query,onOpen}:{addon:Addon;query:string;onOpen:(a:Addon,m:Meta)=>void}){
 ...
 useEffect(()=>{let dead=false;setItems([]);setCatalogError("");setLoading(true);const load=async()=>{try{for(const c of (addon.catalogs||[]).filter(c=>c.type===typeFilter)){if(query&&!c.extra?.some(e=>e?.name==="search"))continue;const extra=query?"&search="+encodeURIComponent(query):"";const r=await fetch("/api/addon/resource?addon="+encodeURIComponent(addon.url)+"&resource=catalog&type="+c.type+"&id="+encodeURIComponent(c.id)+extra);if(!r.ok)continue;const j=await r.json();if(!dead)setItems(x=>{const seen=new Set(x.map(m=>m.id));return [...x,...(j.metas||[]).filter((m:Meta)=>m?.id&&!seen.has(m.id)).slice(0,20)]})}}catch(e){if(!dead)setCatalogError(e instanceof Error?e.message:"Catalog failed")}finally{if(!dead)setLoading(false)}};load();return()=>{dead=true}},[addon.url,query,typeFilter]);
