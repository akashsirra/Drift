...
export default function Home(){
 ...
 return <main>
  ...
  {selected&&<div className="modal"><div className="modalCard"><button className="close" onClick={()=>setSelected(null)}>×</button><div className="detail">{selected.poster&&<img src={selected.poster}/>}<div><span>{selected.type}</span><h2>{selected.name}</h2><p>{selected.description}</p><p>{selected.releaseInfo}</p><div className="actions"><button onClick={library}>＋ Library</button></div>{selected.type==="series"&&selected.videos?.length?<EpisodePicker meta={selected} season={selectedSeason} setSeason={setSelectedSeason} onEpisode={openEpisode}/>:null}<div className="streamStatus">{resolving?"Resolving streams…":notice||"No stream information yet."}</div>{error&&<div className="error">{error}</div>}{streamLinks(streams,selected.name,selected)}</div></div></div></div>}
 </main>
}
