...
function watchHref(s:Stream,title:string,meta?:Meta){const hints:any=s.behaviorHints||{};const subs=(hints.subtitles||hints.subtitle||s.subtitles||[]);const episodeId=(hints as any).videoId||((s as any).season!=null&&(s as any).episode!=null?meta?.id+":"+((s as any).season)+":"+((s as any).episode):""); const params=new URLSearchParams({url:s.url||"",title,ph:btoa(JSON.stringify(hints.proxyHeaders?.request||{})),id:meta?.id||"",type:meta?.type||"",poster:meta?.poster||"",subs:btoa(JSON.stringify(subs)),episodeId,season:String((s as any).season??""),episode:String((s as any).episode??"")});return "/watch?"+params.toString()}
...
export default function Home(){
 ...
 const streamLinks=(items:Stream[],title:string,meta?:Meta)=> <div className="streams">{items.map((s,i)=>s.url?<a key={i} href={watchHref(s,title,meta)} className="stream">{s.name||s.title||"Play"} ▶</a>:s.externalUrl?<a key={i} href={s.externalUrl} target="_blank" rel="noreferrer" className="stream">{s.name||s.title||"Open external"} ↗</a>:<span key={i} className="stream">{s.name||s.title||s.infoHash||"Unsupported stream transport"}</span>)}</div>;
 return <main>
