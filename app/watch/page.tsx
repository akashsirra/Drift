...
function toggleRotate(){setRotate(x=>(x+90)%360);touchControls()}
 ...
 <div className="playerShell" onMouseMove={touchControls} onTouchStart={touchControls} onClickCapture={touchControls}>
 {isHls||isMedia?<video ref={videoRef} className="video" data-zoom={String(zoom)} data-aspect={aspect} data-rotate={String(rotate)} playsInline preload="metadata" poster={poster} onClick={togglePlay}/>:<div className="playerNotice"><h2>Stream resolved</h2><p>This stream uses a transport Drift browser player does not support yet.</p><code>{u}</code></div>}
...
<button onClick={togglePip}>{pip?"▣":"PiP"}</button><button onClick={toggleFullscreen}>{fullscreen?"⤢":"⛶"}</button>{(isMedia||isHls)&&<button className="downloadBtn" onClick={startDownload} title={isHls?"Download HLS as MP4":"Download video"}>⇩</button>}</div></div>}
